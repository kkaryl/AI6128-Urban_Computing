...
def get_data_from_one_txt(txtpath):
    ...
    acce, magn, ahrs, wifi, ibeacon, waypoint = np.array(acce), np.array(magn), np.array(ahrs), np.array(
        wifi), np.array(ibeacon), np.array(waypoint)
    augmented_data = compute_step_positions(acce, ahrs,
                                            waypoint)  # use position estimation funciton in sample code compute_f.py

    index2data = [{'magn': [], 'wifi': defaultdict(list), 'ibeacon': defaultdict(list)} for _ in
                  range(len(augmented_data))]
    ...
    for ibeacon_data in ibeacon:
        ...
        i = np.argmin(tdiff)
        index2data[i]['ibeacon'][ibeacon_data[1]].append(
            int(ibeacon_data[2]))  # 'wifi': {'id2':[-50, -24], 'id5':[-12,-30,-49]}

    txt_data = [None] * len(augmented_data)
    for index in range(len(index2time)):
        t, Px, Py = augmented_data[index]
        txt_data[index] = [t, Px, Py]
        magns, wifis, ibeacons = np.array(index2data[index]['magn']), index2data[index]['wifi'], index2data[index][
            'ibeacon']
        if len(magns) > 0:
...
def split_floor_data(site, floor, testratio=0.1):  # (100 + rssi) / 100  ->  (0,1)
    ...
    for index in train_indices:
        ...
        for bssid, rssi in wifidic.items():
            if bssid not in bssid2index:  # for train set, if a bssid did not appear before, we should add it to a new feature.
                ...
                total_wifi_data = np.concatenate((total_wifi_data, np.zeros((total_wifi_data.shape[0], 1))),
                                                 axis=1)  # add a new feature
        ...
        for uuid, rssi in ibeacondic.items():
            if uuid not in uuid2index:  # for train set, if a uuid did not appear before, we should add it to a new feature.
                ...
                total_ibeacon_data = np.concatenate((total_ibeacon_data, np.zeros((total_ibeacon_data.shape[0], 1))),
                                                    axis=1)  # new feature
    ...
    print()
    print(
        f'There are {no_wifi_ibeacon[0]} steps that did not find wifi, and {no_wifi_ibeacon[1]} steps that did not find ibeacon.')
    print(
        f'There are {not_in_train_wifi_ibeacon[0]} bssids and {not_in_train_wifi_ibeacon[1]} uuids which were detected in testset while not detected in trainset, so that they were discard.')
